...
export default function Logo({
  ...
  return (
    <div className={`flex items-center gap-2.5 ${className ?? ""}`}>
      <svg viewBox="0 0 32 32" className="h-8 w-8 shrink-0" aria-hidden="true">
        <defs>
          <linearGradient id="logo-degrade" x1="0" y1="0" x2="32" y2="32" gradientUnits="userSpaceOnUse">
            <stop offset="0%" stopColor={AGENCIA.colores.degradeInicio} />
            <stop offset="55%" stopColor={AGENCIA.colores.degradeMedio} />
            <stop offset="100%" stopColor={AGENCIA.colores.degradeFin} />
          </linearGradient>
        </defs>
      ...
      <span
        className={`text-lg font-bold tracking-tight ${dark ? "text-white" : "text-neutral-900"}`}
      >
        {AGENCIA.nombre}
      </span>
    </div>
